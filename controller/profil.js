...
module.exports = class {
  ...
  static async getProfile(req, res) {
    ...
    if (!findProfil && !findPendidikan && !findMoto && !findSkillHobi) {
      try {
        ...
        const db = await profil.insertMany({
          createdAt: tanggal,
          pendidikan: d[0]._id,
          moto: b[0]._id,
          skillHobi: c[0]._id,
  ...
  static async editProfile(req, res) {
    ...
    await profil
      .findOneAndUpdate(
        { _id: data._id },
        { name, jabatan, biografi, updatedAt: tanggal, url_img: url },
      )
      .then(() => {
  ...
  static async deleteSkill(req, res) {
    ...
    if (cekInputan) {
      ...
      res.status(400).json({
        message: 'Inputan tidak sesuai dengan data skill. Perhatikan huruf kapital maupun spasi',
      });
    }
  }
